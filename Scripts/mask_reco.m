...
% close all;
addpath('/home/aviadb/Desktop/thesis/sap-voicebox/voicebox/');
reco_dir = '/home/aviadb/Desktop/thesis/beamform_NN_IRM/tf_mask_reco/';
...
%                     end
                    ...
                    out_file = [reco_dir mode '/' set '/irm/' utt '.wav'];
                    audiowrite(out_file, ideal_irm_enh ./ max(abs(ideal_irm_enh)), 16000);
%                     [id_irm_seg_enh, id_irm_glob_enh] = v_snrseg(ideal_irm_enh, clean, 16e3, 'v');
                    ...
                    out_file = [reco_dir mode '/' set '/cirm/' utt '.wav'];
                    audiowrite(out_file, ideal_cirm_enh ./ max(abs(ideal_cirm_enh)), 16000);
...
%                     [id_cirm_seg_enh, id_cirm_glob_enh] = v_snrseg(ideal_cirm_enh, clean, 16e3, 'v');
                    ...
                    out_file = [reco_dir mode '/' set '/psm/' utt '.wav'];
                    audiowrite(out_file, ideal_psm_enh ./ max(abs(ideal_psm_enh)), 16000);
%                     [id_psm_seg_enh, id_psm_glob_enh] = v_snrseg(ideal_psm_enh, clean, 16e3, 'v');
                    ...
                    out_file = [reco_dir mode '/' set '/orm/' utt '.wav'];
                    audiowrite(out_file, ideal_orm_enh ./ max(abs(ideal_orm_enh)), 16000);
